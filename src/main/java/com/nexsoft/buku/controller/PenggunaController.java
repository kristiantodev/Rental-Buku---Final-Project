                ...
                if (m_phone.matches()){
                    if (m_uname.matches()){
                        if(m_pass.matches()){
                            try{
                                if (penggunaService.isUsernameExist(user)) {
                                    logger.error("Tambah admin gagal !! Username sudah digunakan. Silahkan masukan username lain...", user.getUsername());
                                    return new ResponseEntity<>(new CustomErrorType("Tambah admin gagal !!. Username " +
                                            user.getUsername() + " sudah digunakan. Silahkan masukan username lain..."), HttpStatus.CONFLICT);
                                }else  if (penggunaService.isIdUserExist(user)) {
                                    logger.error("Tambah admin gagal !! ID sudah digunakan. Silahkan masukan ID lain...", user.getEmail());
                                    return new ResponseEntity<>(new CustomErrorType("Tambah admin gagal !!. ID " +
                                            user.getIdUser() + " sudah digunakan. Silahkan masukan ID lain..."), HttpStatus.CONFLICT);

                                }else  if (penggunaService.isEmailExist(user)) {
                                    logger.error("Tambah admin gagal !! Email sudah digunakan. Silahkan masukan Email lain...", user.getEmail());
                                    return new ResponseEntity<>(new CustomErrorType("Tambah admin gagal !!. Email " +
                                            user.getEmail() + " sudah digunakan. Silahkan masukan Email lain..."), HttpStatus.CONFLICT);
                                }else  if (penggunaService.isPhoneExist(user)) {
                                    logger.error("Tambah admin gagal !! No.HP sudah digunakan. Silahkan masukan Np.HP lain...", user.getPhone());
                                    return new ResponseEntity<>(new CustomErrorType("Tambah admin gagal !!. No.HP " +
                                            user.getPhone() + " sudah digunakan. Silahkan masukan No.HP lain..."), HttpStatus.CONFLICT);
                                }else{
                                    penggunaService.addAdmin(user);
                                    return new ResponseEntity<>(new CustomSuccessType("Tambah admin berhasil !!"), HttpStatus.CREATED);
                                }
                            }catch (Exception e){
                                e.printStackTrace();
                                return new ResponseEntity<>(new CustomErrorType("Failed create user"), HttpStatus.BAD_REQUEST);
